...
const MEDIA_FIRST_STORES = {
  ...PHOTO_ONLY_STORES,
  mediaItems:
    "id, sourcePostId, sourceIndex, type, sourceKind, creatorHandle, createdAt, updatedAt",
  mediaPreferences:
    "mediaId, visibility, rating, hiddenAt, updatedAt, *localTags",
};
...
export class AppDatabase extends Dexie {
  ...
  mediaPreferences!: Table<MediaPreference, string>;
  ...
  constructor() {
    ...
    this.version(2)
      ...
      .upgrade(async (transaction) => {
        ...
        const removedPostIds = new Set(
          storedPosts
            .filter(
              (post) =>
                post.type !== "post" ||
                !isInstagramPhotoPostUrl(post.canonicalUrl),
            )
            .map((post) => post.id),
        );
        ...
        await posts
          .toCollection()
          .modify((post: SavedPost & { type?: string }) => {
            delete post.type;
          });

        const emptyCollectionIds: string[] = [];
        await collections.toCollection().modify((collection: Collection) => {
    ...
    this.version(3)
      .stores(MEDIA_FIRST_STORES)
      .upgrade(async (transaction) => {
        const posts = (await transaction
          .table("posts")
          .toArray()) as SavedPost[];
        const mediaItems = transaction.table("mediaItems");
        const now = new Date().toISOString();

        if (posts.length > 0) {
          await mediaItems.bulkPut(
            posts.map((post) => createFallbackMediaItem(post, now)),
          );
        }
      });
  }
...
export function createFallbackMediaItem(
  post: SavedPost,
  ...
  return {
    ...
    sourceIndex: 0,
    type: "image",
    sourceKind: post.embedThumbnailUrl ? "remote" : "embed",
    creatorHandle: post.embedAuthorName,
    caption: post.description ?? post.title,
    previewUrl: post.embedThumbnailUrl,
    assetUrl: post.embedThumbnailUrl,
    createdAt: timestamp,
    updatedAt: timestamp,
  };
}
